...
def add_weather_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def temp_min(m):
        ...
        return base
    def rainfall(m):
        if m in MONSOON_MONTHS: return np.random.gamma(2, 400) if m in [7, 8] else np.random.gamma(2, 200)
        elif m in POST_MONSOON_MONTHS: return np.random.exponential(50)
...
def prepare_features(df: pd.DataFrame) -> pd.DataFrame:
    """Apply full feature engineering pipeline to historical data."""
...
def generate_trajectory(df_latest: pd.DataFrame, days: int = 14) -> List[Dict]:
    """Generate day-by-day trajectory for each commodity-market."""
    # Use full feature set from prepared data
    df = prepare_features(df_history.copy())
    ...
    for _, row in df_latest.iterrows():
        ...
        # Get full feature row for this group
        group_data = df[
        ...
        trajectories.append({
            ...
            "market": row["market"],
            "current_price": round(float(row["modal_price"]), 2),
            "trajectory": traj
        })
